...
static bool IsBatteryPoweroff(uint16_t mv) {return mv<=3150;}

static bool IsBatteryLow(uint16_t mv) {return mv<=3660;}
/* Check INTERNAL value for shut off battery level  - around 3.00V */
static bool IsBatteryDead(uint16_t mv) {return mv<3400;}
/* battery full is indicated in firmware for this chip*/
static bool IsBatteryMid(uint16_t mv) {return mv>3730;}
static bool IsBatteryHIGH(uint16_t mv) {return mv>3800;}
static bool IsBatteryFull(uint16_t mv) {return mv>=4170;}
...
{ 
  /* x  current value*/
	...
	else
	{
	   ...
	   if (IsBatteryLow(IDDmeas))
	            {                                	           
			system_flag_table.batt_Status  =  BATT_LOW;
        	    }
    	  else if (IsBatteryFull(IDDmeas))
                {
