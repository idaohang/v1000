...
long  temperature = 0xffffffff;
long  pressure= 0xffffffff;;
long Altitude= 0xffffffff;;											//º£°Î¸ß¶È
...
void bmp085Convert(void)
{
	...
	unsigned long b4, b7;
	long temperature_new, pressure_new;
...
//	up = bmp085ReadPressure();  // ¶ÁÈ¡Ñ¹Ç¿
	...
	b5 = x1 + x2;
	 ...
	 if(temperature == 0xffffffff)	 	
	 	...
	 	{
		temperature_new = (temperature +((b5 + 8) >> 4))/2;
...
//	printf("\r\n pressure_tmep: %f m \n\n",pressure_tmep);
	...
	altitude*=100;	 		//×ª»»³ÉÀåÃ×µ¥Î»µÄ¸ß¶ÈÖµ£¬µ÷ÓÃÊ±ÔÙ»»Ëã³É´øĞ¡ÊıµÄ¸ß¶ÈÖµ£¬Ìá¸ß¾«¶È
//	if(altitude<0)
//		altitude	
//	conversion(altitude);
...
{
	//static long Tempnum = 0;									//ÁÙÊ±Öµ
//	 u8 flag;					 //¼ÆËãº£°Î¸ß¶ÈÊ¹ÓÃµ½µÄÊı¾İ±êÖ¾
//	u8 dtbuf_jian[50];
extern system_flag system_flag_table;
extern nmea_msg gpsx; 	
//if(Altitude == 0xffffffff)
...
//	Altitude=Altitude/(1+(temperature-15)/288.15);
//	printf("Altitude1: %d m \r\n",Altitude);

#if 0	
	if(Altitude>=0) 
		;
//		flag = 0;
	else												  	//Èç¹ûº£°Î¸ß¶ÈÊÇÒ»¸ö¸ºÊı£¬ÒªÏÈÈ¡·´
	{
	//	flag = 1;
		Altitude=~Altitude;
	}
      ...
      if(system_flag_table.ajust_Arr_flag == 1)
      		{
	  	Altitude = Altitude-(system_flag_table.ajust_arr*100);
      		}
	  else
	  	{
...
#endif	
	//printf("system_flag_table.ajust_arr: %d m \n\n",system_flag_table.ajust_arr);
	//Altitude=(Altitude+BMP085_Get_Altitude())/2;
