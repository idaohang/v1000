...
//lat1 = floor(lat1)+ ((lat1-floor(lat1))/60*100);
//lng1 = floor(lng1)+ ((lng1-floor(lng1))/60*100);
//printf("\r\n lat1  %.5f",lat1);
//printf("\r\n lng1  %.5f",lng1);
	
//lat2 = floor(lat2)+ (lat2-floor(lat2))/60;
//lng2 = floor(lng2)+ (lng2-floor(lng2))/60;
...
float A_Ec, A_Ed,B_Ec,B_Ed,angle;
...
//   out = sqrt(dx * dx + dy * dy);
 ...
 //   printf("\r\n longitude2  %.5f",longitude2);
   // printf("\r\n longitude1  %.5f",longitude1);
    //printf("\r\n latitude2  %.5f",latitude2);
    //printf("\r\n latitude1  %.5f",latitude1);	
     // printf("\r\n angle111  %.1f",angle);
...
float get_angle(float longitude1, char longitude1_flag, float latitude1, char latitude1_flag,
...
float shita1,shita2,fai1,fai2,x1,x2,y1,y2,z1,z2,x,y,z,angle;
   ...
   //   Ec = Rj + (Rc - Rj) * (90.- latitude1) / 90.;
    //  Ed = Ec * cos(fai2);
     ...
     angle = atan(fabs(z/sqrt(x*x + y*y)))/PI*180;
...
float * quad(float ym, float yz, float yp)
{
	...
	float a, b, c, dis, dx, xe, ye, z1, z2, nz;
...
void find_moonrise_set(float mjd, float tz, float glong, float glat,int dls,int ST,PSunMoonTime moonTimes)
{
	...
	float  sglat, date, ym, yz,  utrise, utset,sinho,cglat,xe,ye;
