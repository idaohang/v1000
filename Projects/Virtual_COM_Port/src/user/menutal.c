...
//aconst u8 *COMPILED_DATE=__DATE__;//ªÒµ√±‡“Î»’∆⁄
...
uc8 menu7[]  ={"7.≥±œ´±Ì"};
uc8 menu8[]  ={"8.…Ë÷√≤Àµ•"};
uc8 menu8_a[]  ={"1,.◊¯±Í±Ì æ∑Ω Ω"};
uc8 menu8_b[] ={"2,æ‡¿Îµ•Œª"};
uc8 menu8_c[] ={"3,Œ¬∂»µ•Œª"};
uc8 menu8_d[] ={"4,º«¬ºº‰∏Ù"};
uc8 menu8_e[] ={"5,◊‘∂®“Âœ‘ æ“≥√Ê"};
uc8 menu8_f[] ={"6,∏¸∏ƒœµÕ≥”Ô—‘"};
uc8 menu8_g[] ={"7, ±«¯…Ë÷√"};
...
typedef struct
{
 u8 MenuID;      //±æ≤Àµ•◊¥Ã¨À˜“˝∫≈
 u8 ParMenuID;    //∏∏≤Àµ•◊¥Ã¨À˜“˝∫≈
 uc8 *MenuName;  //≤Àµ•Œƒ±æƒ⁄»›
 void (*CurrentOperate)(void); //±æ≤Àµ•µƒπ¶ƒ‹∫Ø ˝
 u8 subnumber;      //±æ≤Àµ•◊¥Ã¨À˜“˝∫≈
}KeyTabStruct;
//≤Àµ•±Ì£¨
...
void convert_show_en(float tp,u8 flag,u8 y,u8 mode)
{
	...
	extern const u8 bmpfen[];
      ...
      if(system_flag_table.myunit.COORD == 0)
		...
		if(mode == 1 )
			{
			if(miao<100)
				sprintf((char *)dtbuf,"%1c  %.6f",flag,miao);	//µ√µΩæ≠∂»◊÷∑˚¥Æ	
			else	
				sprintf((char *)dtbuf,"%1c %.6f",flag,miao);	//µ√µΩæ≠∂»◊÷∑˚¥Æ	
			}
		else
		...
		if(mode == 1 )
			...
			if(du >100)
				{
					sprintf((char *)dtbuf,"%1c %03d",flag,du);	//µ√µΩæ≠∂»◊÷∑˚¥Æ	
				}
			else
				{
					sprintf((char *)dtbuf,"%1c  %02d",flag,du);	//µ√µΩæ≠∂»◊÷∑˚¥Æ
		...
		if(miao<10)
			sprintf((char *)dtbuf,"0%.3f",miao);	//µ√µΩæ≠∂»◊÷∑˚¥Æ	
		else	
			sprintf((char *)dtbuf,"%.3f",miao);	//µ√µΩæ≠∂»◊÷∑˚¥Æ	
		index += 8;
	...
	//tp=gpsx.longitude;
		...
		Movie_Show_Img(index,y,bmpdu);
		index += 8;
		sprintf((char *)dtbuf,"%02d",fen);	//µ√µΩæ≠∂»◊÷∑˚¥Æ
		OLED_ShowString(index,y,dtbuf);
		index +=16;	
		Movie_Show_Img(index,y,bmpfen);
		index += 8;
		if(miao<10.0)
			sprintf((char *)dtbuf,"0%.1f",miao);	//µ√µΩæ≠∂»◊÷∑˚¥Æ
		else	
			sprintf((char *)dtbuf,"%.1f",miao);	//µ√µΩæ≠∂»◊÷∑˚¥Æ
		OLED_ShowString(index,y,dtbuf);	
		index =120;
...
void convert_show_en_input(u8 *prt_c ,u8 y)
{
	...
	sprintf((char *)dtbuf,"%1c%c%c%c",prt_c[0],prt_c[1],prt_c[2],prt_c[3]);	//µ√µΩæ≠∂»◊÷∑˚¥Æ
	index = 56;
	...
	Movie_Show_Img(index,y,bmpdu);
	index += 8;
	sprintf((char *)dtbuf,"%c%c",prt_c[4],prt_c[5]);	//µ√µΩæ≠∂»◊÷∑˚¥Æ
	OLED_ShowString(index,y,dtbuf);
	index +=16;	
	Movie_Show_Img(index,y,bmpfen);
	index += 8;
	sprintf((char *)dtbuf,"%c%c.%c",prt_c[6],prt_c[7],prt_c[8]);	//µ√µΩæ≠∂»◊÷∑˚¥Æ
	OLED_ShowString(index,y,dtbuf);	
	index =120;
	Movie_Show_Img(index,y,bmpmaio);
}
...
{ 
 ...
 //	 sprintf((char *)dtbuf,"%02d:%02d",RTC_TimeStructure.RTC_Hours,RTC_TimeStructure.RTC_Minutes);	//œ‘ æUTC»’∆⁄
...
//	show_levev_space();
	...
	if(system_flag_table.lang == 1)
 	...
 	tp =gpsx.pdop;
 	tp =tp/100;
	 sprintf((char *)dtbuf,"%0.1f",tp);	//œ‘ æUTC»’∆⁄
	OLED_ShowString(72, 48, dtbuf);
//	Movie_Show_Img(0,16,bmp_jinwei);
 //      printf("longitude :%d \r\n",gpsx.longitude);	//œ‘ æUTC»’∆⁄
   //    printf("latitude :%d \r\n",gpsx.latitude);	//œ‘ æUTC»’∆⁄
...
if((system_power_flag == SYSTEM_POWER_LOW)&&(system_flag_table.run_mode == 0))
	  ...
	  if(system_flag_table.myunit.TIMER == 0)	 	
	 		{
	 		  if(gpsx_utc>=12*60)
				 ...
				 if(gpsx_utc/60>=10)
				 	...
				 	sprintf((char *)dtbuf,"%d:%02d:%02d",gpsx_utc/60-10,gpsx_utc%60,RTC_TimeStructure.RTC_Seconds);	//œ‘ æUTC»’∆⁄
				 	}
				 else
				 	{
				 	Movie_Show_Img(1,19,Bmp_bigpm);
			 		sprintf((char *)dtbuf,"%d:%02d:%02d",gpsx_utc/60,gpsx_utc%60,RTC_TimeStructure.RTC_Seconds);	//œ‘ æUTC»’∆⁄
				 ...
				 if(gpsx_utc/60>=10)
				 	...
				 	{
				 	sprintf((char *)dtbuf,"%d:%02d:%02d",2,gpsx_utc%60,RTC_TimeStructure.RTC_Seconds);	//œ‘ æUTC»’∆⁄
					 Movie_Show_Img(1,19,Bmp_bigam_1);
					 }
				 else
				 	{
			...
			{	
			 sprintf((char *)dtbuf,"%02d:%02d:%02d",RTC_TimeStructure.RTC_Hours,RTC_TimeStructure.RTC_Minutes,RTC_TimeStructure.RTC_Seconds);	//œ‘ æUTC»’∆⁄
	...
	//		 sprintf((char *)dtbuf,"%d",ddrunmv);	//œ‘ æUTC»’∆⁄
		 ...
		 if(system_flag_table.myunit.TIMER == 0)	 	
	 		{
	 		  if(gpsx_utc>=12*60)
				 ...
				 if(gpsx_utc>=13*60)
			 		gpsx_utc= gpsx_utc - 12*60;
		 		sprintf((char *)dtbuf,"%02d:%02d",gpsx_utc/60,gpsx_utc%60);	//œ‘ æUTC»’∆⁄
				 OLED_ShowString_head(0,0,dtbuf);	
				 ...
				 if(gpsx_utc>60)
					...
					sprintf((char *)dtbuf,"%02d:%02d",12,gpsx_utc%60);	//œ‘ æUTC»’∆⁄
				 OLED_ShowString_head(0,0,dtbuf);	
...
//		year = year-2000;
	...
	switch(system_flag_table.myunit.DATA)
		{
		case 0:
		 	sprintf((char *)dtbuf,"%02d-%02d-%02d",year,month,data);	//œ‘ æUTC»’∆⁄
		 	break;
		case 1:
		 	sprintf((char *)dtbuf,"%02d-%02d-%02d",month,data,year);	//œ‘ æUTC»’∆⁄
		 	break;
		case 2:
		 	sprintf((char *)dtbuf,"%02d-%02d-%02d",data,month,year);	//œ‘ æUTC»’∆⁄
		 	break;
	...
	if(system_flag_table.myunit.TIMER == 1)
		{
		sprintf((char *)dtbuf,"%02d:%02d:%02d",hour,min,sec);	//œ‘ æUTC ±º‰
...
//		time_zone = comp_time;
		sprintf((char *)dtbuf,"+%02d:%02d",comp_time,(gpsx_utc-rtc_timer-(comp_time*60)));	//œ‘ æUTC ±º‰
		}
	else
...
//		time_zone = comp_time;
		sprintf((char *)dtbuf,"+%02d:%02d",comp_time,(rtc_timer-gpsx_utc-(comp_time*60)));	//œ‘ æUTC ±º‰
		}
#endif			
...
//		sprintf((char *)dtbuf,"%c%c.%c C",bai,shi,ge);	//µ√µΩæ≠∂»◊÷∑˚¥Æ
                sprintf((char *)dtbuf,"%.1f C",temperature_F/10);	//µ√µΩæ≠∂»◊÷∑˚¥Æ 
	...
	//	if(temperature_F>=1000)
			{
			sprintf((char *)dtbuf,"%.1f F",temperature_F/10);	//µ√µΩæ≠∂»◊÷∑˚¥Æ
...
//			Movie_Show_Img(88,16,bmpdu);
	...
	//		Movie_Show_Img(96,16,bmpdu);
		...
		Movie_Show_Img((112-what_is_numbe_10(temperature_F/10)*8),16,bmpdu);

      if(system_flag_table.myunit.H_W == 0)
      	{

	sprintf((char *)&dtbuf,"%dm",Altitude/100);	//µ√µΩæ≠∂»◊÷∑˚¥Æ		
      	}
		...
		sprintf((char *)&dtbuf,"%dft",gpsx_utc);	//µ√µΩæ≠∂»◊÷∑˚¥Æ	
      	}
...
//	show_levev_space();
	...
	if(system_flag_table.myunit.H_W == 0)//||(system_flag_table.myunit.H_W == 1))
	 	sprintf((char *)dtbuf,"%.1fkm/h",tp/1000);		    		//µ√µΩÀŸ∂»◊÷∑˚¥Æ	 

#if 1
	else if(system_flag_table.myunit.H_W == 1)
	 	sprintf((char *)dtbuf,"%.3fmi/h",tp/1609);		    		//µ√µΩÀŸ∂»◊÷∑˚¥Æ	 
#endif
	else if(system_flag_table.myunit.H_W == 2)
	 	sprintf((char *)dtbuf,"%.1fkt/h",tp/1852);		    		//µ√µΩÀŸ∂»◊÷∑˚¥Æ	 
 	OLED_ShowString(48,16,dtbuf);	 
      ...
      if(system_flag_table.myunit.H_W == 0)
		...
		sprintf((char *)&dtbuf,"%dft",tmp_Att);	//µ√µΩæ≠∂»◊÷∑˚¥Æ	
      		}
	...
	if(tp <10)
		sprintf((char *)dtbuf,"   %d",(u16)tp);		    		//µ√µΩÀŸ∂»◊÷∑˚¥Æ	 
	else if(tp <100)
		sprintf((char *)dtbuf,"  %d",(u16)tp);		    		//µ√µΩÀŸ∂»◊÷∑˚¥Æ	 
	else
		sprintf((char *)dtbuf," %d",(u16)tp);		    		//µ√µΩÀŸ∂»◊÷∑˚¥Æ	 
...
//(81-what_is_numbe_10(temperature)*8)
		...
		Movie_Show_Img((112-what_is_numbe_10(temperature_F/10)*8),0,bmpdu);
		
	//	conversion(most_temperature);
//		sprintf((char *)dtbuf,"%c%c.%c",bai,shi,ge);	//most
		sprintf((char *)dtbuf,"%d",temperature/10);	//µ√µΩæ≠∂»◊÷∑˚¥Æ
	...
	//	conversion(temperature_F);
		...
		if(temperature_F>=1000)
			{
			sprintf((char *)dtbuf,"%.1f F",temperature_F);	//µ√µΩæ≠∂»◊÷∑˚¥Æ
		...
		if(temperature_F>=1000)
			{
			sprintf((char *)dtbuf,"%c%c%c",qian,bai,shi);	//µ√µΩæ≠∂»◊÷∑˚¥Æ
			}
		else
			{
			sprintf((char *)dtbuf,"%c%c.%c ",bai,shi,ge);	//µ√µΩæ≠∂»◊÷∑˚¥Æ
			}
		OLED_ShowString12(0,16,dtbuf);	
		...
		OLED_ShowString12(0,46,dtbuf);	
		#endif
		sprintf((char *)dtbuf,"%d",temp/10);	//µ√µΩæ≠∂»◊÷∑˚¥Æ
		OLED_ShowString12(0,16,dtbuf);		
...
//		conversion(pressure);
		...
		sprintf((char *)dtbuf,"%.1fhpa",pressure_unit/100);	//µ√µΩæ≠∂»◊÷∑˚¥Æ
		OLED_ShowString(57,0,dtbuf);
	//	conversion(most_pressure);
		sprintf((char *)dtbuf,"%d",(int)(most_pressure/100));	//µ√µΩæ≠∂»◊÷∑˚¥Æ
		OLED_ShowString12(0,16,dtbuf);
	//	conversion(lest_pressure);
		sprintf((char *)dtbuf,"%d",(int)(lest_pressure/100));	//µ√µΩæ≠∂»◊÷∑˚¥Æ
		OLED_ShowString12(0,46,dtbuf);	
		}
	...
	//	conversion(pressure);
	pressure_unit = pressure;
		sprintf((char *)dtbuf,".1fmba",pressure_unit/100);	//µ√µΩæ≠∂»◊÷∑˚¥Æ
		OLED_ShowString(57,0,dtbuf);
		//conversion(most_pressure);
		sprintf((char *)dtbuf,"%d",(int)(most_pressure/100));	//µ√µΩæ≠∂»◊÷∑˚¥ÆÆ
		OLED_ShowString12(0,16,dtbuf);
	//	conversion(lest_pressure);
		sprintf((char *)dtbuf,"%d",(int)(lest_pressure/100));	//µ√µΩæ≠∂»◊÷∑˚¥Æ
		OLED_ShowString12(0,46,dtbuf);	
	...
	else if(system_flag_table.myunit.PREE == 2)
		{
			...
			sprintf((char *)dtbuf,"%.1fmmhg",pressure_unit);	//µ√µΩæ≠∂»◊÷∑˚¥Æ
			OLED_ShowString(57,0,dtbuf);
			pressure_unit = most_pressure/1.333;
			//		conversion(pressure_unit);
			sprintf((char *)dtbuf,"%d",(int)(pressure_unit));	//µ√µΩæ≠∂»◊÷∑˚¥Æ
			OLED_ShowString12(0,16,dtbuf);
...
//	const unsigned char Bmp_pree_word[];
	...
	if(old_altiude_div_cnt == 0)
      ...
      if(system_flag_table.myunit.H_W == 0)
      		{
			sprintf((char *)&dtbuf,"%ldm",Altitude/100);	//µ√µΩæ≠∂»◊÷∑˚¥Æ
			OLED_ShowString(65,0,dtbuf);				

			sprintf((char *)dtbuf,"%ld",most_Altitude/100);		    		//µ√µΩÀŸ∂»◊÷∑˚¥Æ	 
			OLED_ShowString12((24-what_is_numbe_10(most_Altitude/100)*6),16,dtbuf);

			sprintf((char *)dtbuf,"%ld",lest_Altitude/100);		    		//µ√µΩÀŸ∂»◊÷∑˚¥Æ	 
			OLED_ShowString12((24-what_is_numbe_10(lest_Altitude/100)*6),46,dtbuf);	
		//	printf("\r\nlest_Altitude= %ld",lest_Altitude);	
      		}
     else  if(system_flag_table.myunit.H_W >= 1)
      		{
			sprintf((char *)&dtbuf,"%.1fft",(Altitude/100/0.3045));	//µ√µΩæ≠∂»◊÷∑˚¥Æ
			OLED_ShowString(65,0,dtbuf);							

			sprintf((char *)dtbuf,"%.0f",(floor)(most_Altitude/30.45));		    		//µ√µΩÀŸ∂»◊÷∑˚¥Æ	 
			OLED_ShowString12((24-(what_is_numbe_10(most_Altitude/30.45)*6)),16,dtbuf);

			sprintf((char *)dtbuf,"%.0f",(floor)(lest_Altitude/30.45));		    		//µ√µΩÀŸ∂»◊÷∑˚¥Æ	 
...
//	extern const u8 bmp_speed[];
	...
	if(system_flag_table.myunit.H_W == 0)
		{
			sprintf((char *)dtbuf,"%dkm/h",(u32)tp/1000);		    		//µ√µΩÀŸ∂»◊÷∑˚¥Æ	 
			OLED_ShowString(70,0,dtbuf);	 
			tp=most_speed;	   
			sprintf((char *)dtbuf,"%.1f",tp);		    		//µ√µΩÀŸ∂»◊÷∑˚¥Æ	 
			OLED_ShowString12(0,16,dtbuf);
			tp=lest_speed;	   
			sprintf((char *)dtbuf,"%.1f",tp);		    		//µ√µΩÀŸ∂»◊÷∑˚¥Æ	 
			OLED_ShowString12(0,46,dtbuf);	
		...
		{
			sprintf((char *)dtbuf,"%dmi/h",(u32)tp/1609);		    		//µ√µΩÀŸ∂»◊÷∑˚¥Æ	 
			OLED_ShowString(70,0,dtbuf);	 
			tp=most_speed;	   
			sprintf((char *)dtbuf,"%.1f",tp/1.609);		    		//µ√µΩÀŸ∂»◊÷∑˚¥Æ	 
			OLED_ShowString12(0,16,dtbuf);
			tp=lest_speed;	   
			sprintf((char *)dtbuf,"%.1f",tp/1.609);		    		//µ√µΩÀŸ∂»◊÷∑˚¥Æ	 
			OLED_ShowString12(0,46,dtbuf);	
		...
		{
			sprintf((char *)dtbuf,"%dkt/h",(u32)tp/1852);		    		//µ√µΩÀŸ∂»◊÷∑˚¥Æ	 
			OLED_ShowString(70,0,dtbuf);	 
			tp=most_speed;	   
			sprintf((char *)dtbuf,"%.1f",tp/1.852);		    		//µ√µΩÀŸ∂»◊÷∑˚¥Æ	 
			OLED_ShowString12(0,16,dtbuf);
			tp=lest_speed;	   
			sprintf((char *)dtbuf,"%.1f",tp/1.852);		    		//µ√µΩÀŸ∂»◊÷∑˚¥Æ	 
			OLED_ShowString12(0,46,dtbuf);	
...
void menu_3d_seq(void)
{
	...
	while(i>0&&flag==1)
	{
		flag=0;                  /* ÊØèË∂üÊéíÂ∫èÂâçÊ†áÂøóflagÁΩÆ0 */ 
		for(j=0;j<i;j++) 
...
//	      sprintf((char *)dtbuf,"%02d",home_menu_item_index+menu_item_index);		    		//µ√µΩÀŸ∂»◊÷∑˚¥Æ	
...
//		Movie_Show_Img(48, 16,bmpjuli);
	...
	if(distance <=0.01)
		...
		if(system_flag_table.myunit.H_W == 0)
			...
			if(distance>=10 && distance<100)
				sprintf((char *)dtbuf,"%.2fkm",distance);		    		//µ√µΩÀŸ∂»◊÷∑˚¥Æ	
			else if (distance<10)
				sprintf((char *)dtbuf,"%.3fkm",distance);		    		//µ√µΩÀŸ∂»◊÷∑˚¥Æ	
			else
				sprintf((char *)dtbuf,"%ldkm",(u32)(distance));		    		//µ√µΩÀŸ∂»◊÷∑˚¥Æ
			}
		else if(system_flag_table.myunit.H_W == 1)
			{
			if((distance/1.609)>=10 && (distance/1.609)<100)
				sprintf((char *)dtbuf,"%.2fmi",distance/1.609);		    		//µ√µΩÀŸ∂»◊÷∑˚¥Æ	
			else if ((distance/1.609)<10)
				sprintf((char *)dtbuf,"%.3fmi",distance/1.609);		    		//µ√µΩÀŸ∂»◊÷∑˚¥Æ	
			else
				sprintf((char *)dtbuf,"%ldmi",(u32)(distance/1.609));		    		//µ√µΩÀŸ∂»◊÷∑˚¥Æ
			}
#if 1		
		else if(system_flag_table.myunit.H_W == 2)
			{
			if((distance/1.852)>=10 && (distance/1.852)<100)
				sprintf((char *)dtbuf,"%.2fkt",distance/1.852);		    		//µ√µΩÀŸ∂»◊÷∑˚¥Æ	
			else if ((distance/1.852)<10)
				sprintf((char *)dtbuf,"%.3fkt",distance/1.852);		    		//µ√µΩÀŸ∂»◊÷∑˚¥Æ	
			else
				sprintf((char *)dtbuf,"%ldkt",(u32)(distance/1.852));		    		//µ√µΩÀŸ∂»◊÷∑˚¥Æ			
	...
	if(system_flag_table.myunit.H_W == 0)//||(system_flag_table.myunit.H_W == 1))
		{
		if(tp/1000>=100.0)
			sprintf((char *)dtbuf,"%.0fkm/h",tp/=1000);		    		//µ√µΩÀŸ∂»◊÷∑˚¥Æ	 
		else
		 	sprintf((char *)dtbuf,"%.1fkm/h",tp/=1000);		    		//µ√µΩÀŸ∂»◊÷∑˚¥Æ	 
		}
#if 1
		...
		{
		if(tp/1609>=100.0)
			sprintf((char *)dtbuf,"%.0fmi/h",tp/=1609);		    		//µ√µΩÀŸ∂»◊÷∑˚¥Æ	 
		else
		 	sprintf((char *)dtbuf,"%.1fmi/h",tp/=1609);		    		//µ√µΩÀŸ∂»◊÷∑˚¥Æ	 
		}
#endif
	else if(system_flag_table.myunit.H_W == 2)
		{
		if(tp/1852>=100.0)
			sprintf((char *)dtbuf,"%.0fkt/h",tp/=1852);		    		//µ√µΩÀŸ∂»◊÷∑˚¥Æ	 
		else
		 	sprintf((char *)dtbuf,"%.1fkt/h",tp/=1852);		    		//µ√µΩÀŸ∂»◊÷∑˚¥Æ	 	 
		}
 ...
 //     tp = distance/(tp/1000);
  //    timer_need = (tp - floor(tp))*60;
	...
	if((tp == 0.0)||(timer_need>=6000))
      ...
      sprintf((char *)dtbuf,"%02d:%02d",timer_need/60,timer_need%60);		    		//µ√µΩÀŸ∂»◊÷∑˚¥Æ	
				...
				if(system_flag_table.myunit.H_W == 0)
					...
					if((distance/1.852)>=10 && (distance/1.852)<100)
						...
						sprintf((char *)dtbuf,"%ldkt",(u32)(distance/1.852));		    		//µ√µΩÀŸ∂»◊÷∑˚¥Æ			
                         ...
                         if(gpsx.fixmode >1)  // GPS∂®ŒªµΩ∫Û≤≈œ‘ æƒ⁄»›
					OLED_ShowString(48,16,dtbuf);	
				}
			break;
	      	case 1:
	      	//	 home_flag = 1; // Ω¯»Î—°‘Ò¥Ê¥¢–À»§µ„ƒ£ Ω
			//home_flag = 3;	// Ω¯»Î ‰»Îƒ£ Ω	
                    if(system_flag_table.lang == 1)
...
//	cout &lt;&lt; sunrise &lt;&lt; endl; 
	...
	sunriset( year, month, day, lon, lat,  
		altit, upper_limb, &sunrise, &sunset );//∫À–ƒÀ„∑®£¨º∆À„≥ˆ»’≥ˆ»’¬‰ ±º‰±£¥Ê‘⁄sunrise∫Õsunset¿Ô 
...
//	 printf("\r\n sunrise  %d:%d",hour,minute);
      ...
      if(gpsx.fixmode!=3)
      		...
      		sprintf((char *)dtbuf,"%02d:%02d",hour,minute);	//œ‘ æUTC»’∆⁄
	OLED_ShowString(22,16,dtbuf);		

//    sprintf((char *)dtbuf,"%d:%d",hour,minute);	//œ‘ æUTC»’∆⁄
...
//	OLED_ShowString_head(0,0,dtbuf);	
	...
	sprintf((char *)dtbuf,"%02d-%02d-%02d",my_timer.w_year ,my_timer.w_month,my_timer.w_date);	//œ‘ æUTC»’∆⁄
	OLED_ShowString(0,48,dtbuf);		    
...
//	if((KeyTab[keyTabIndex].MenuID != MENU_TRANSFORMATION)&&(KeyTab[keyTabIndex].ParMenuID != MENU_TRANSFORMATION))
		...
		if(baifenbi == 100)
			...
			{
			sprintf((char *)dtbuf,"%02d",baifenbi);		    		//µ√µΩÀŸ∂»◊÷∑˚¥Æ	
	...
	switch(guji_mode )
		...
		case RECORED_START:			
			...
			//	stm_write_eerpom(GUJI_STRAT_JINDU, gpsx.longitude);
				...
				//	sprintf((char *)dtbuf,"%d",interst_pos_number);		    		//µ√µΩÀŸ∂»◊÷∑˚¥Æ	
		...
		case RECORED_T:
			...
			//	interst_pos_number = 0;
			interst_pos_number++;
		//	sprintf((char *)dtbuf,"%02d",interst_pos_number);		    		//µ√µΩÀŸ∂»◊÷∑˚¥Æ	
	...
	//tp_lati/=1000000;
		...
		if(system_flag_table.lang == 1)
			...
			distance	 = (floor)(guiji_distance);
				...
				{
			//	sprintf((char *)dtbuf,"%dkm",distance);		    		//µ√µΩÀŸ∂»◊÷∑˚¥Æ		
				if(system_flag_table.myunit.H_W == 0)
					{
					if(guiji_distance>1.0)
					 	sprintf((char *)dtbuf,"%.2fkm",guiji_distance);		    		//µ√µΩÀŸ∂»◊÷∑˚¥Æ	 
                                else
					...
					if((guiji_distance/1.609)>1.0)
						...
						sprintf((char *)dtbuf,"%dft",((u32)(guiji_distance*1000/0.3048)));		    		//µ√µΩÀŸ∂»◊÷∑˚¥Æ	 
						//µ√µΩÀŸ∂»◊÷∑˚¥Æ	 
					}
					...
					if((guiji_distance/1.852)>1.0)
					 	sprintf((char *)dtbuf,"%.2fkt",guiji_distance/1.852);		    		//µ√µΩÀŸ∂»◊÷∑˚¥Æ	 				
					else						 	
						sprintf((char *)dtbuf,"%dft",((u32)(guiji_distance*1000/0.3048)));		    		//µ√µΩÀŸ∂»◊÷∑˚¥Æ	 
					}
				OLED_ShowString(0,48,dtbuf);
			...
			sprintf((char *)dtbuf,"%02dh%02dm",h,m);		    		//µ√µΩÀŸ∂»◊÷∑˚¥Æ	
			OLED_ShowString(80,48,dtbuf);
			if(guji_icon_index == 0)
				...
				sprintf((char *)dtbuf,"%d",tp);		    		//µ√µΩÀŸ∂»◊÷∑˚¥Æ
				...
				{
				sprintf((char *)dtbuf,"%d",Message_head_number);		    		//µ√µΩÀŸ∂»◊÷∑˚¥Æ
					...
					if(system_flag_table.myunit.H_W == 0)
						...
						if(menu_setting_postion_2<=2)
							...
							//	system_flag_table.guji_record.by_distance_vaule= arry_bydistance[i];
								sprintf((char *)dtbuf,"%dm",arry_bydistance[i]);		    		//µ√µΩÀŸ∂»◊÷∑˚¥Æ
						...
						if(menu_setting_postion_2<=2)
							...
							for(i =0;i<3;i++)
								...
								sprintf((char *)dtbuf,"%dft",arry_bydistance_2[i]);		    		//µ√µΩÀŸ∂»◊÷∑˚¥Æ
								OLED_ShowString12(16,(29+(i*12)),dtbuf);
								}
							}
						else
...
//						stm_read_eerpom(MENU_SETTING_GUJI_BY_TIME ,&setting_tp);
						...
						if(menu_setting_postion_2<=2)
							...
							//	system_flag_table.guji_record.by_time_vaule= arry_time[i];
								sprintf((char *)dtbuf,"%ds",arry_time[i]);		    		//µ√µΩÀŸ∂»◊÷∑˚¥Æ
								OLED_ShowString12(16,(29+(i*12)),dtbuf);
								}
							}
						else
				...
				case BY_SPEED:
						...
						if(system_flag_table.myunit.H_W == 0)
							...
							#endif	
							sprintf((char *)dtbuf,"%dkm",menu_setting_postion_2);		    		//µ√µΩÀŸ∂»◊÷∑˚¥Æ
							}
						else if(system_flag_table.myunit.H_W == 1)
							{
							#if 0
							...
							#endif
							sprintf((char *)dtbuf,"%dmi",menu_setting_postion_2);		    		//µ√µΩÀŸ∂»◊÷∑˚¥Æ
							}
						else
							{
							sprintf((char *)dtbuf,"%dkt",menu_setting_postion_2);		    		//µ√µΩÀŸ∂»◊÷∑˚¥Æ
							}
						#endif
					//	
