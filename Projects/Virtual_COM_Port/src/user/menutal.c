...
//		conversion(pressure);
	...
	//	conversion(lest_pressure);
	...
	else if(system_flag_table.myunit.PREE == 2)
		{
			...
			sprintf((char *)dtbuf,"%.1fmmhg",pressure_unit/100);	//µ√µΩæ≠∂»◊÷∑˚¥Æ
			OLED_ShowString(57,0,dtbuf);
			pressure_unit = most_pressure/1.333;
			//		conversion(pressure_unit);
			sprintf((char *)dtbuf,"%d",(int)(pressure_unit/100));	//µ√µΩæ≠∂»◊÷∑˚¥Æ
			OLED_ShowString12(0,16,dtbuf);
	...
	else if(system_flag_table.myunit.PREE == 3)
		{
			...
			sprintf((char *)dtbuf,"%.2fmmhg",pressure_unit/100);	//µ√µΩæ≠∂»◊÷∑˚¥Æ	
			OLED_ShowString(57,0,dtbuf);
...
//				sprintf((char *)&dtbuf,"%.1fft",(Altitude/100/0.3048));	//µ√µΩæ≠∂»◊÷∑˚¥Æ
   //                    else		  
				sprintf((char *)&dtbuf,"%.0fft",(Altitude/100/0.3048));	//µ√µΩæ≠∂»◊÷∑˚¥Æ					   	
			OLED_ShowString(65,0,dtbuf);							

			sprintf((char *)dtbuf,"%.0f",(most_Altitude/30.48));		    		//µ√µΩÀŸ∂»◊÷∑˚¥Æ	 
			OLED_ShowString12((24-(what_is_numbe_10(most_Altitude/30.48)*6)),16,dtbuf);

			sprintf((char *)dtbuf,"%.0f",lest_Altitude/30.48);		    		//µ√µΩÀŸ∂»◊÷∑˚¥Æ	 
